...
/**
 * @brief       S型加减速运动
 * @param       vo:初速度;vt:末速度;AcTime:加速时间;DeTime:减速时间;step:步数;count：校验位；（count自己加的，用于消掉内存不足）
 * @retval      无
 */
void stepmotor_move_rel(int32_t vo, int32_t vt, float AcTime,float DeTime,int32_t step)
{
    ...
    if(step < 0)
    {
		GPIO_SetBits(GPIOC,GPIO_Pin_10);	//PC10为使能引脚，输出高电平
    ...
    {
		GPIO_SetBits(GPIOC,GPIO_Pin_10);	//PC10为使能引脚，输出高电平
		GPIO_ResetBits(GPIOD,GPIO_Pin_7);	//PD7为方向引脚，输出低电平	
    }
	...
	TIM_SetCompare2(TIM3,(uint16_t)(g_toggle_pulse/2));			//这里的通道设置错误导致不能连续转
...
void speed_decision_s()
{
	...
	{	
        ...
        if(i == 2)              /* 2次，说明已经输出一个完整脉冲 */
        {
            ...
            switch(g_motor_sta)
            {
                case STATE_ACCEL:		//加速状态
					TIM_CCxCmd(TIM3,TIM_Channel_2,TIM_CCx_Enable);		//打开输出通道
                    g_add_pulse_count++;
                    Tim_Pulse = T1_FREQ_S / (*g_calc_t.ptr);          /* 由速度表得到每一步的定时器计数值 */
...
//					TIM_CCxCmd(TIM3,TIM_Channel_2,TIM_CCx_Enable);		//打开输出通道
                ...
                case STATE_STOP:			//停止状态
					TIM_ITConfig(TIM3,TIM_IT_CC2,DISABLE);		//关闭定时器3的通道2中断
                    g_motor_sta = STATE_IDLE;
					TIM_ITConfig(TIM3,TIM_IT_CC2,DISABLE);
					TIM_CCxCmd(TIM3,TIM_Channel_2,TIM_CCx_Disable);		//关闭输出
                    break;
...
//					TIM_CCxCmd(TIM3,TIM_Channel_2,TIM_CCx_Disable);		//关闭输出
			...
			{
				myfree(SRAMIN,g_calc_t.decel_tab);          /* 运动完要释放内存 */
				myfree(SRAMIN,g_calc_t.accel_tab);          /* 释放减速段速度表 */
				g_motor_sta = STATE_STOP;
			}
        }
		
		/*  设置比较值 */
		TIM_ClearITPendingBit(TIM3, TIM_IT_CC2);	// 清除定时器中断	
